...
<div class="subscription">
	...
	<h2>Подписаться на рассылку</h2>
			...
			<div class="rubrics">
				<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
						<input type="checkbox" id="RUBRIC_<?echo $itemID?>" name="RUB_ID[]" value="<?=$itemValue["ID"]?>"<?if($itemValue["CHECKED"]) echo " checked"?> class="checkbox_input"/>
						<label for="RUBRIC_<?echo $itemID?>"><?echo $itemValue["NAME"]?><span class="checkbox_<?=$itemID?>"><?echo $itemValue["DESCRIPTION"]?></span></label>
				<?endforeach;?>
				<div class="clear"></div>
			</div>
				<?if($arResult["ID"]==0):?>
	...
	<div class="subscription-utility">
		...
		<input name="sf_EMAIL" type="text" class="subscription-textbox" value="<?echo GetMessage("CT_BSE_EMAIL")?>" onblur="if (this.value=='')this.value='<?echo GetMessage("CT_BSE_EMAIL")?>'" onclick="if (this.value=='<?echo GetMessage("CT_BSE_EMAIL")?>')this.value=''" /> <input type="submit" class="form__submit" value="<?echo GetMessage("CT_BSE_BTN_SEND")?>" />
	</div>
	</form>
	<?endif?>
