...
class ProductReserves
{
	...
	protected function processingOrders(Main\Type\DateTime $earliestUpdatedAt = null)
	{
		...
		if ($earliestUpdatedAt !== null && $this->inNearNow($earliestUpdatedAt))
		{
			$processingStatuses = $this->getProcessingStatuses();

			$filter['VALUE'] = array_map(static function($status) { return $status . '%'; }, $processingStatuses);
			$filter['VALUE'][] = $statusService::VIRTUAL_CREATED;
		}
	...
	protected function inNearNow(Main\Type\DateTime $date)
	{
		$gap = 60;
		$now = new Main\Type\DateTime();

		return ($now->getTimestamp() - $date->getTimestamp() <= $gap);
	}

	protected function getProcessingStatuses()
	{
		$statusService = $this->provider->getStatus();
		$result = [];

		foreach ($statusService->getProcessOrder() as $status => $order)
		{
			if ($statusService->isShipped($status)) { break; }
