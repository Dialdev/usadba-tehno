...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/catalog/admin/cat_discount_edit.php");
...
if (!$bReadOnly && $_SERVER['REQUEST_METHOD']=="POST" && strlen($Update)>0 && check_bitrix_sessid())
{
	...
	if (array_key_exists('CAT_IDS', $_POST) && is_array($_POST['CAT_IDS']))
	{
	...
	$arPaysID = array();
	if (array_key_exists('PAY_IDS', $_POST) && is_array($_POST['PAY_IDS']))
	{
		foreach ($_POST['PAY_IDS'] as &$intValue)
		{
			$intValue = intval($intValue);
			if ($intValue > 0)
			{
	...
	if (!$bVarsFromForm)
	{
		...
		{
			//COption::SetOptionString("altasib.discounts", "iblockid_for_discount_".$ID, implode(",", $arIblockID));
			COption::SetOptionInt("altasib.discounts", "period_for_discount_".$ID, trim(htmlspecialchars($_REQUEST["PERIOD_CALC"])));
			COption::SetOptionInt("altasib.discounts", "limit_for_discount_".$ID, trim(htmlspecialchars($_REQUEST["LIMIT"])));
			
			//      Add/Update Group Users Discounts
			...
			if(!$valGroup)
			{
				...
				$arFields = Array(
					"ACTIVE"       => "Y",
					"C_SORT"       => 100,
					"NAME"	 => GetMessage("ALTASIB_DISCOUNTS_NAME_GR").$NAME,
					"DESCRIPTION"  => "ALTASIB_DISCOUNTS",
				);
				$valGroup = $group->Add($arFields);
				if (strlen($group->LAST_ERROR)>0)
				{
					$errorMessage .= $group->LAST_ERROR."<br>";
					$bVarsFromForm = true;
					$DB->Rollback();
				}
			}
			else
			{
				...
				$group->Update($valGroup, $arFields);
			}
			...
			if($valGroup)
			{
				$arFieldsUp = array(
					...
					"GROUP_IDS" => array($valGroup),
					//"CATALOG_GROUP_IDS" => $arCatalogGroupID,				
				);
				$res = CCatalogDiscount::Update($ID, $arFieldsUp);
			}

			COption::SetOptionInt("altasib.discounts", "group_for_discount_".$ID, $valGroup);

			if(!empty($arPaysID))
			{
				COption::SetOptionString("altasib.discounts", "pays_for_discount_".$ID, implode(";",$arPaysID));
			}
			...
			if (!$bVarsFromForm)
			{
				if (strlen($apply)<=0)
				{
					if (!empty($return_url))
						...
						LocalRedirect("/bitrix/admin/altasib_discounts_list.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false));
				}
				else
				{
					LocalRedirect("/bitrix/admin/altasib_discounts_edit.php?lang=".LANGUAGE_ID."&ID=".$ID.GetFilterParams("filter_", false).'&'.$tabControl->ActiveTabParam());
				}
			}
		}
	}
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/bitrix/admin/altasib_discounts_list.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false)
	)
);
...
if ($ID > 0)
{
	...
	if (!$bReadOnly)
	{
		...
		$aMenu[] = array(
			...
			"ICON" => "btn_new",
			"LINK" => "/bitrix/admin/altasib_discounts_edit.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false)
		);
		if (!$boolCopy)
		{
			$aMenu[] = array(
				"TEXT"=>GetMessage("BT_CAT_DISCOUNT_EDIT_CONT_NAME_COPY"),
				"LINK"=>"/bitrix/admin/altasib_discounts_edit.php?ID=".$ID."&action=copy&lang=".LANGUAGE_ID.GetFilterParams("filter_", false),
				"ICON"=>"btn_copy",
			);
			...
			$aMenu[] = array(
				...
				"ICON" => "btn_delete",
				"LINK" => "javascript:if(confirm('".GetMessageJS("CDEN_DELETE_DISCOUNT_CONFIRM")."')) window.location='/bitrix/admin/altasib_discounts_list.php?action=delete&ID[]=".$ID."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."#tb';",
				"WARNING" => "Y"
			);
		}
	}
...
<?/*	    COSTOM FIELD DEFAULTS		   */?>
<input type="hidden" name="VALUE_TYPE" value="P">
<input type="hidden" name="NOTES" value="ALTASIB_DISCOUNTS">
...
$tabControl->Begin(array(
	"FORM_ACTION" => '/bitrix/admin/altasib_discounts_edit.php?lang='.urlencode(LANGUAGE_ID),
));
...
$tabControl->BeginNextFormTab();
	...
	$val = COption::GetOptionInt("altasib.discounts", "period_for_discount_".$ID, 12);
	$tabControl->AddEditField("PERIOD_CALC", GetMessage("ALTASIB_DISCOUNTS_NAME_PERIOD_TR").":", true, array('size' => 20, 'maxlength' => 20), $val);
	
	$val = COption::GetOptionInt("altasib.discounts", "limit_for_discount_".$ID, "");
	$tabControl->AddEditField("LIMIT", GetMessage("ALTASIB_DISCOUNTS_NAME_LIMIT_TR").":", true, array('size' => 20, 'maxlength' => 20), $val);
...
$tabControl->BeginNextFormTab();
	...
	$tabControl->BeginCustomField("PAY_IDS", GetMessage('DSC_PAYS_TYPES').":", false);
	
	$arDiscountPayList = explode(";", $valGroup = COption::GetOptionString("altasib.discounts", "pays_for_discount_".$ID));
	?><tr id="tr_PAY_IDS">
		<td valign="top" width="40%"><? echo $tabControl->GetCustomLabelHTML(); ?></td>
		<td valign="top" width="60%">
			<select name="PAY_IDS[]" multiple size="8">				
			<?
				$dbPays = CSalePaySystem::GetList(array("NAME" => "ASC"), array());
				while ($arPay = $dbPays->Fetch())
				{
					?><option value="<?= $arPay["ID"] ?>"<?if (in_array(intval($arPay["ID"]), $arDiscountPayList)) echo " selected";?>>[<?= $arPay["ID"] ?>] <?= htmlspecialcharsEx($arPay["NAME"]) ?></option><?
				}
			?></select>
		</td>
	</tr><?
	if ($ID > 0 && !empty($arDiscountPayList))
	{
		$arHidden = array();
		foreach ($arDiscountPayList as &$value)
		{
			if (0 < intval($value))
				$arHidden[] = '<input type="hidden" name="PAY_IDS[]" value="'.intval($value).'">';
		}
		if (isset($value))
			unset($value);
		$strHidden = implode('',$arHidden);
	...
	{
		$strHidden = '<input type="hidden" name="PAY_IDS[]" value="">';
	}
	$tabControl->EndCustomField("PAY_IDS",
		$strHidden
	);
