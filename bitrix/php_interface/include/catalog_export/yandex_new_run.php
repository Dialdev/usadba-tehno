		...
		if ($bParam)
		{
			if (is_array($description))
			{
				foreach ($value as $key => $val)
				{
					...
					$strProperty .= '<param name="'.yandex_text2xml($description[$key], true).'" unit="">'.yandex_text2xml($val, true).'</param>';
				}
			}
			else
			{
				$strProperty .= '<param name="'.yandex_text2xml($arProperties[$PROPERTY]['NAME'], true).'" unit="">'.yandex_text2xml($value, true).'</param>';
			}
		}
		else
		{
...
/*					$minPrice = $arPrice['DISCOUNT_PRICE'];
	...
	elseif ($arCatalog['CATALOG_TYPE'] == CCatalogSKU::TYPE_PRODUCT || $arCatalog['CATALOG_TYPE'] == CCatalogSKU::TYPE_FULL)
	{
		$arOfferSelect = array(
			...
			"PREVIEW_PICTURE", "DETAIL_TEXT", "DETAIL_TEXT_TYPE", "DETAIL_PICTURE", "DETAIL_PAGE_URL",
...
/*						$minPrice = $arPrice['DISCOUNT_PRICE'];
				...
				foreach ($arYandexFields as $key)
				{
					...
					case 'description':
						$strValue =
							"<description><![CDATA[".yandex_text2xml($arItem["~DETAIL_TEXT"])."]]></description>\n";
						break;
					case 'param':
						if (is_array($XML_DATA) && is_array($XML_DATA['XML_DATA']) && is_array($XML_DATA['XML_DATA']['PARAMS']))
						{
